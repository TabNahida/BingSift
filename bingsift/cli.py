...
from .parser import parse_html, build_bing_url
from .filters import filter_results

def main():
    parser = argparse.ArgumentParser(prog="bingsift", description="Parse and filter Bing SERP HTML into JSON/CSV.")
    ...
    p_parse.add_argument("--include", nargs="*", help="Keywords that must appear")
    p_parse.add_argument("--exclude", nargs="*", help="Keywords that must not appear")
    p_parse.add_argument("--allow-domain", dest="allow_domain", nargs="*", help="Only keep results in these domains")
    p_parse.add_argument("--deny-domain", dest="deny_domain", nargs="*", help="Drop results in these domains")
